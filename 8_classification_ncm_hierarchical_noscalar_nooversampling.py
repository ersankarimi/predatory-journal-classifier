...
METHOD_NAME = "bfs"
CLASSIFICATION_DIR = f"8_classification/ncm_hierarchical/{METHOD_NAME}_ncm_hierarchical_noscalar_nooversampling"
os.makedirs(CLASSIFICATION_DIR, exist_ok=True)

LOG_FILE = os.path.join(CLASSIFICATION_DIR, f"8_{METHOD_NAME}_ncm_hierarchical_noscalar_nooversampling.log")
...
def train_model(X_train, y_train, n_clusters=2, linkage="ward"):
    ...
    return None, clustering, clf, cluster_mapping, silhouette_avg
...
def predict_and_evaluate(scaler, clustering, clf, cluster_mapping, X_test, y_test):
    ...
    # ~~Penskalaan Fitur pada Data Uji~~ (DIHAPUS)
...
def save_model(model, scaler, clustering, cluster_mapping, silhouette_avg, model_path):
    joblib.dump((model, clustering, cluster_mapping, silhouette_avg), model_path)
    logging.info(
...
def load_model(model_path):
    try:
        model, clustering, cluster_mapping, silhouette_avg = joblib.load(
            model_path
        )
        logging.info(
            f"✅ Model, clustering, cluster_mapping, dan silhouette score dimuat dari {model_path}"
        )
        return model, clustering, cluster_mapping, silhouette_avg, None
...
if __name__ == "__main__":
    ...
    train_vector_path = f"6_vectorized_journal_data/6_{METHOD_NAME}_train_vectors.npy"
    train_label_path = f"6_vectorized_journal_data/6_{METHOD_NAME}_train_labels.npy"
    ...
    model_path = os.path.join(
        CLASSIFICATION_DIR, f"8_{METHOD_NAME}_ncm_hierarchical_noscalar_nooversampling_model.pkl" # nama model diubah
    )
